...
struct QuickLogView: View {
    ...
    private var logForm: some View {
        VStack(alignment: .leading, spacing: 14) {
            VStack(alignment: .leading, spacing: 6) {
                Label(editingFlightID == nil ? "Flight Entry" : "Edit Flight", systemImage: editingFlightID == nil ? "paperplane.fill" : "square.and.pencil")
                    .font(.headline)
            ...
            Button {
                ...
                Label(editingFlightID == nil ? "Save Flight" : "Update Flight", systemImage: editingFlightID == nil ? "plus.circle.fill" : "checkmark.circle.fill")
                    .frame(maxWidth: .infinity)
            }
    ...
    private var fieldLogForm: some View {
        VStack(alignment: .leading, spacing: 16) {
            Label(editingFlightID == nil ? "Quick Flight" : "Edit Flight", systemImage: "bolt.fill")
                .font(.title3.bold())
            ...
            Button {
                ...
                Label(editingFlightID == nil ? "Log Flight Now" : "Update Flight", systemImage: "paperplane.fill")
                    .font(.headline)
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 8)
            }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        HStack(alignment: .top) {
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                if let comparison = comparisonText(for: flight) {
                                    Text(comparison)
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                }
    ...
    private func saveFlight() {
        ...
        let wasEditing = editingFlightID != nil
        if let editingFlightID {
            store.updateFlight(
                id: editingFlightID,
                rocket: rocket,
    ...
    private func comparisonText(for flight: Flight) -> String? {
        guard let previous = store.flights.first(where: { $0.id != flight.id && $0.rocketID == flight.rocketID && $0.flownAt < flight.flownAt }) else {
            return nil
        }
    ...
    private func loadFlightIntoForm(_ flight: Flight) {
        editingFlightID = flight.id
        selectedRocketID = flight.rocketID
        selectedMotorDesignation = flight.motorDesignation
        mass = flight.rocketMassGrams
        wantedAltitude = flight.targetAltitudeFeet ?? store.targetAltitudeFeet
        altitude = flight.measuredAltitudeFeet
        flightTimeSeconds = flight.flightTimeSeconds ?? store.syncedCompetitionInfo.flightTimeRange.lowerBound
        temperature = flight.weather.temperatureF
        wind = flight.weather.windMPH
        humidity = flight.weather.humidityPercent
        location = flight.weather.location
        parachute = flight.parachuteSizeInches
    ...
    private func rocketName(for flight: Flight) -> String {
        store.rockets.first(where: { $0.id == flight.rocketID })?.name ?? "Unknown Rocket"
    }

    @ViewBuilder
    private func flightScoreBadge(for flight: Flight) -> some View {
        let summary = store.scoreSummary(for: flight)
